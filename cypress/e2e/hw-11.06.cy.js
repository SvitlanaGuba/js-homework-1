 ...
 describe('testing "Toastr" submenu', () => {
     ...
     testCases.forEach(({testData, expectedResults}, index) => {
         it(`Verify testData${index + 1}`, () => {
             ...
             cy.get('input[name="title"]', {timeout: 2000}).clear({force: true}).type(testData[0].title, {force: true});
             cy.get('input[name="content"]', {timeout: 2000}).clear({force: true}).type(testData[0].content);
             cy.get('input[name="timeout"]', {timeout: 2000}).clear({force: true}).type(testData[0].time);
